const anzhiyu = {
  ...
  sayhi: function () {
    ...
    const getTimeState = () => {
      ...
      if (hour >= 0 && hour <= 5) {
        message = "睡个好觉，保证精力充沛(～﹃～)~zZ";
      } else if (hour > 5 && hour <= 10) {
        message = "一日之计在于晨( •̀ ω •́ )✧";
      } else if (hour > 10 && hour <= 14) {
        message = "吃饱了才有力气干活(ง •_•)ง";
      } else if (hour > 14 && hour <= 18) {
        message = "集中精力，攻克难题(๑•̀ㅂ•́)و✧";
      } else if (hour > 18 && hour <= 24) {
        message = "不要太劳累了，早睡更健康哦o(*￣▽￣*)ブ";
      }
  ...
  getCustomPlayList: function () {
    ...
    const urlParams = new URLSearchParams(window.location.seaCusrch);
    const userId = "9173908460";
